...
const register = async (req, res, next) => {
  try {
    ...
    await log({ userId: user._id, action: 'register', resource: 'user', detail: 'Đăng ký tài khoản mới', ip: req.ip })
...
const login = async (req, res, next) => {
  try {
    ...
    const token = generateToken(user._id)
    await log({ userId: user._id, action: 'login', resource: 'user', detail: 'Đăng nhập hệ thống', ip: req.ip })
...
const logout = async (req, res) => {
  await log({ userId: req.user._id, action: 'logout', resource: 'user', detail: 'Đăng xuất', ip: req.ip }).catch(() => {})
...
const forgotPassword = async (req, res, next) => {
  try {
    ...
    const user = await User.findOne({ email: email.toLowerCase() })
    if (!user) return success(res, {}, 'Nếu email tồn tại, link đã được gửi')
...
const changePassword = async (req, res, next) => {
  try {
    ...
    await user.save()
    await log({ userId: req.user._id, action: 'change_password', resource: 'user', detail: 'Đổi mật khẩu', ip: req.ip })
