...
export default function AdminPage() {
  ...
  const handleChangeRole = async () => {
    ...
    try {
      ...
      alert(err.response?.data?.message || 'Lỗi đổi role')
    }
  }
  ...
  const handleDelete = async (user) => {
    ...
    try {
      ...
      alert(err.response?.data?.message || 'Lỗi xoá người dùng')
    }
  }
  ...
  return (
    <div className="view-container">
      ...
      <div className="admin-body">
        ...
        {/* Table */}
        {loading ? (
          <div className="board-loading">Đang tải...</div>
        ) : (
                  ...
                  <tr key={user._id}>
                    ...
                    <td>
                      <div className="admin-actions">
                        ...
                        {/* Khoá/mở */}
                        <button
                          className={`admin-btn ${user.isActive ? 'lock' : 'unlock'}`}
                ...
                <div className="role-picker">
                  {Object.entries(roleLabel).map(([value, label]) => (
                    ...
                    >
                      ...
                      {label}
                      {value === 'admin' && <span style={{ fontSize: 10, opacity: .7 }}> (toàn quyền)</span>}
                      {value === 'pm'    && <span style={{ fontSize: 10, opacity: .7 }}> (quản lý DA)</span>}
                      {value === 'member'&& <span style={{ fontSize: 10, opacity: .7 }}> (thành viên)</span>}
                    </button>
                  ))}
                </div>
              </div>
