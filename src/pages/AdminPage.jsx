...
export default function AdminPage() {
  ...
  const loadActivity = async (user) => {
    setShowActivity(user); setActLoading(true)
    try {
                    ...
                    <tr key={user._id}>
                      ...
                      <td>
                        <div className="admin-actions">
                          <button className="admin-btn edit" title="Lịch sử hoạt động"
                            onClick={() => loadActivity(user)}>📋</button>
                          <button className="admin-btn edit" title="Đổi role"
        ...
        <div style={{ padding: '0 20px' }}>
          <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: 24 }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 16 }}>
              Quản lý tên, logo, múi giờ, chính sách mật khẩu và các tính năng của hệ thống.
            </div>
            <button className="btn-primary" onClick={() => setShowWorkspace(true)}>
              Mở cài đặt Workspace
            </button>
          </div>
        </div>
      )}
            ...
            <div className="modal-footer">
              <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
                ...
                <button className="btn-primary" onClick={handleChangeRole}
                  disabled={newRole === editUser.role}>Lưu</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showActivity && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowActivity(null)}>
          <div className="modal" style={{ maxWidth: 560 }}>
            <div className="modal-header">
              <h2 className="modal-title">Lịch sử hoạt động — {showActivity.name}</h2>
            ...
            <div className="modal-body" style={{ maxHeight: 400, overflowY: 'auto' }}>
              {actLoading ? (
                ...
                <div style={{ textAlign: 'center', padding: 24, color: 'var(--text-faint)' }}>Chưa có hoạt động nào</div>
              ) : activity.map(log => (
                <div key={log._id} style={{ display: 'flex', gap: 10, padding: '8px 12px',
                  background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 8, marginBottom: 6 }}>
