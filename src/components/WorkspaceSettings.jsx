...
export default function WorkspaceSettings({ onClose }) {
  ...
  useEffect(() => {
    api.get('/workspace').then(r => setSettings(r.data?.data || null)).finally(() => setLoading(false))
  }, [])
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose?.()}>
      <div className="modal" style={{ maxWidth: 560 }}>
        <div className="modal-header">
          <h2 className="modal-title">Cấu hình Workspace</h2>
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', padding: '0 20px' }}>
          {[['general','Chung'],['password-policy','Chính sách MK'],['features','Tính năng']].map(([id, label]) => (
        ...
        <div className="modal-body">
          {msg && (
            <div style={{ padding: '8px 12px', borderRadius: 8, marginBottom: 12, fontSize: 13,
              background: msg.startsWith('Lỗi') ? 'rgba(239,68,68,.15)' : 'rgba(34,197,94,.15)',
              color:      msg.startsWith('Lỗi') ? '#ef4444'             : '#22c55e',
              border: '1px solid ' + (msg.startsWith('Lỗi') ? 'rgba(239,68,68,.3)' : 'rgba(34,197,94,.3)') }}>
              {msg}
            </div>
          )}
  ...
  return (
    <form onSubmit={e => { e.preventDefault(); onSave(form) }}>
      <div className="modal-field">
        <label className="modal-label">Tên Workspace</label>
...
function PasswordPolicyTab({ policy, onSave, saving }) {
  ...
  return (
    <form onSubmit={e => { e.preventDefault(); onSave(form) }}>
      <div className="modal-field">
        <label className="modal-label">Độ dài mật khẩu tối thiểu</label>
      ...
      <div className="modal-field">
        <label className="modal-label">Session timeout (phút)</label>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
        {[['requireUppercase','Yêu cầu chữ hoa'],['requireNumber','Yêu cầu chữ số']].map(([key, label]) => (
...
function FeaturesTab({ features, onSave, saving }) {
  ...
  const featureList = [
    ['chat',    'Chat nhóm (UC25)'],
    ['finance', 'Quản lý ngân sách (UC26–30)'],
    ['sprint',  'Sprint & Burndown (UC15–16)'],
    ['report',  'Báo cáo & Thống kê (UC31–34)'],
  ]
  return (
    <form onSubmit={e => { e.preventDefault(); onSave(form) }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
          ...
          <div key={key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            ...
            <label style={{ position: 'relative', display: 'inline-flex', cursor: 'pointer' }}>
              ...
              <div style={{ width: 44, height: 24, borderRadius: 999,
                background: form[key] ? 'var(--accent)' : 'var(--border)', position: 'relative', transition: 'background .2s' }}>
